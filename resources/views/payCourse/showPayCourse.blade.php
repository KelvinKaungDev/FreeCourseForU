...
@section("content")
        <div class="album">
            <div class="container border border-warning mt-3 mb-5">
                <h1 class=" font-weight-normal text-warning text-center">Pay Courses</h1>
            </div>
            <div class="container">
                <div class="row">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            @if(\Illuminate\Support\Facades\Auth::check())
                                                @if(\Illuminate\Support\Facades\Auth::user()->hasRole("Admin"))
                                                    <div class="btn-group">
                                                        <a href="{{url("admin/deleteCourse/".$course->id)}}" class="btn btn-sm btn-danger">Delete</a>
                                                        <a href="{{url("admin/editCourse/".$course->id)}}" class="btn btn-sm text-white btn-warning">Edit</a>
                                                    </div>
                                                @else
                                                    <div class="btn-group">
                                                        <a href="{{url("/viewCourse/".$course->id)}}" class="btn btn-sm text-white btn-warning">ViewDetail</a>
                                                        <a href="tel:{{$course->ph_number}}" class="btn btn-sm text-white btn-primary">Buy</a>
                                                    </div>
                                                @endif  
                                            @else
                                                <div class="btn-group">
                                                    <a href="{{ route('login') }}" class="btn btn-sm text-white btn-warning">ViewDetail</a>
                                                    <a href="{{ route('login') }}" class="btn btn-sm text-white btn-primary">Learn</a>
                                                </div>
                                            @endif
                                            <small class="text-warning font-weight-bold">({{$course->price}})</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
